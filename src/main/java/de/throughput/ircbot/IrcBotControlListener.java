...
public class IrcBotControlListener extends ListenerAdapter {
        ...
        if (parts.length == 2) {
            ...
            if (COMMAND_AUTH.equals(command)) {
                ...
                System.out.printf("AUTH %s with pw hash %s\n", event.getUser()
                        .getNick(), pwhash.toString(16));
                if (pwhash.equals(this.admins.get(event.getUser()
                        .getNick()))) {
                    System.out.printf("AUTH %s successful\n", event.getUser()
                            .getNick());
                    putAuth(event.getUser()
                            .getNick());
                    event.respond("yes");
                } else {
                    event.respond("no");
    ...
    private void onUnrecognizedPrivateMessage(GenericMessageEvent event) {
        ...
        this.authedAdmins.entrySet()
                .stream()
                .filter(e -> e.getValue()
                        .plusMinutes(AUTH_TIMEOUT_MINUTES)
                        .isAfter(LocalDateTime.now()))
                .map(e -> e.getKey())
                .filter(admin -> !admin.equals(event.getUser()
                        .getNick()))
                .forEach(admin -> {
                    event.getBot()
                            .send()
                            .message(admin, String.format("%s: %s", event.getUser()
    ...
    private boolean isAuth(String nick) {
        ...
        if (lastseen != null && lastseen.plusMinutes(AUTH_TIMEOUT_MINUTES)
                .isAfter(LocalDateTime.now())) {
            putAuth(nick);
            return true;
        }
