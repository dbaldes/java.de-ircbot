...
public class QuoteCommandHandler implements CommandHandler, MessageHandler {
  ...
  private void quote(CommandEvent command, String nick) {
    if (nick.equals(command.getEvent().getBot().getNick())) {
      command.respond("I'm afraid I can't do that.");
    } else {
      String key = command.getEvent().getChannelSource() + ":" + nick;
  ...
  private void quoteRandom(CommandEvent command) {
    lookupRandomQuote(command.getEvent().getChannelSource())
      .ifPresentOrElse(
        quote -> command.getEvent().getChannel().send().message(quote.getAsMessage()),
        () -> command.respond(MSG_NO_QUOTE_FOUND));
  }

  private void quoteRandom(CommandEvent command, String nick) {
    if (nick.equals(command.getEvent().getBot().getNick())) {
      command.getEvent().getChannel().send().message(botQuote());
    } else {
      lookupRandomQuote(command.getEvent().getChannelSource(), nick)
        .ifPresentOrElse(
          quote -> command.getEvent().getChannel().send().message(quote.getAsMessage()),
          () -> command.respond(MSG_NO_QUOTE_FOUND));
    }
  ...
  private void storeMessage(MessageEvent event) {
    QuoteMessage msg = new QuoteMessage(
        event.getChannelSource(), event.getUser().getNick(),
  ...
  private void evictOldMessages() {
    ...
    if (now - lastEvictionTimestamp > ONE_MINUTE) {
      ...
      lastMessages.entrySet().removeIf(
          entry -> now - entry.getValue().getTimestamp() > ONE_HOUR);
    }
  }
