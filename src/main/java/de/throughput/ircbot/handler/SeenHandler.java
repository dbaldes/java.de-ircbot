...
public class SeenHandler implements CommandHandler, MessageHandler {
  ...
  public boolean onMessage(MessageEvent event) {
    upsert(event.getChannelSource().toLowerCase(), event.getUser().getNick().toLowerCase(), event.getMessage());
    return false;
  }
  ...
  public boolean onCommand(CommandEvent command) {
    if (CMD_SEEN.equals(command.getCommand())) {
      command.getArgLine()
        ...
        .ifPresentOrElse(
          nick -> command.respond(seen(command.getEvent().getChannelSource(), nick)),
