...
public class AviationInfoCommandHandler implements CommandHandler {
  ...
  public boolean onCommand(CommandEvent command) {
    ...
    if (command.getCommand().equals(CMD_FLIGHT)) {
      saneFlightNumber(command.getArgLine()).ifPresentOrElse(
          ...
          () -> usage(command));
      handled = true;
    } else if (command.getCommand().equals(CMD_AIRCRAFT)) {
      saneAircraftRegistration(command.getArgLine()).ifPresentOrElse(
          ...
          () -> usage(command));
      handled = true;
    } else if (command.getCommand().equals(CMD_AIRPORT)) {
      saneAirportCode(command.getArgLine(), 
          ...
          () -> usage(command));
      handled = true;
    } else if (command.getCommand().equals(CMD_AVHERALD)) {
  ...
  private Optional<String> saneFlightNumber(Optional<String> input) {
    return input
        .map(flightNumber -> flightNumber.replace(" ", "").toUpperCase(Locale.ROOT))
